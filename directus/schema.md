...
Записи сортируются по `sort`. Первые две — маленькие карточки сверху; третья (последняя) — большая карточка «Общий парк…» внизу блока (её `badge` на сайте не выводится).
